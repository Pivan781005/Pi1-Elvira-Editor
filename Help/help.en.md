...
Opening/previewing does not create a backup. First persistent font/data/graphics work preserves immutable original O-files where applicable, for example RUNVGAO.EXE/RUNITO.EXE, GAMEPCO, or 012O.VGA. Launcher generation uses `ELVIRA.BAT` -> `ELVIRA.BAK`: BAK is immutable, never overwritten/deleted, and Restore copies it back to the active BAT.
...
`RUNEGA` and EGA executable/font patching are outside the supported scope. **The editor never modifies RUNEGA, and V5/V2 patches do not apply to it.**

## [about] About / Credits
**π1 Elvira I & II Editor v1.0** supports VGA graphics editing, GAMEPC text/data variants, extended CP852-compatible fonts, Mods & Launcher, and a portable DOS launcher helper. Glyph `0x81` is protected as the HUD erase glyph; RUNEGA/EGA is unsupported and never modified.
