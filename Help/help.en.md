...
Opening/previewing does not modify the original game. Normal Text/Graphics/Font/Runtime UI project edits are saved to project state. Build Variant materializes supported changes into owned `VARIANTS\<runtime>\<edition>\`. The normal project workflow does not write modified game EXEs or O-files into the pristine GameRoot. Launcher BAT backup/restoration remains a controlled explicit exception where applicable. Recovery/Safety operations are explicit and scoped.
