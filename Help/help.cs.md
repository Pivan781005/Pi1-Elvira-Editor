...
Otevření/náhled nemění originální hru. Běžné projektové úpravy textů/grafiky/fontů/Runtime UI se ukládají do stavu projektu. Sestavit variantu materializuje podporované změny do vlastněných `VARIANTS\<runtime>\<edition>\`. Běžný projektový postup nezapisuje upravené herní EXE ani O-soubory do nedotčeného GameRoot. Zálohování/obnova launcher BAT zůstává řízenou výslovnou výjimkou, kde to platí. Operace obnovy/bezpečnosti jsou výslovné a ohraničené.
