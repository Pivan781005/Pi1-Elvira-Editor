...
internal sealed record RestorePlanItem(RestorePlanItemKind Kind, string RelativePath, string Detail, BuiltInVariantId? VariantId = null);
...
internal sealed class RecoverySafetyService
{
    ...
    public RecoverySafetyOperationResult RemoveOwnedVariant(ProjectContext project, VariantContext variant)
    {
        VariantDirectoryOperationResult result = _directories.DeleteVariantDirectory(project, variant);
        return result.Status == VariantDirectoryOperationStatus.Deleted
            ? new(true, "Editor-owned variant directory removed.", result)
            : new(false, "Variant directory was not removed: " + result.Status + ".", result);
    }

    public RecoverySafetyOperationResult RebuildOwnedVariant(ProjectContext project, VariantContext variant)
    {
        ...
        if (ownership.Status != VariantDirectoryOperationStatus.AlreadyValid)
            return new(false, "Variant rebuild requires an existing editor-owned directory: " + ownership.Status + ".", ownership);
        ...
        return build.Status == CompositeBuildStatus.Success
            ? new(true, "Owned variant rebuilt.", ownership, build)
            : new(false, "Variant rebuild did not complete: " + build.Status + ".", ownership, build);
    }
    ...
    public RestorePlan CreateRestorePlan(ProjectContext project)
    {
        ...
        foreach (VariantDirectoryInfo directory in _directories.Enumerate(project, known).OrderBy(item => item.DirectoryKey, StringComparer.OrdinalIgnoreCase))
        {
            if (directory.Classification == VariantDirectoryClassification.KnownOwnedVariant && directory.VariantId is { } variantId)
                items.Add(new(RestorePlanItemKind.RemoveOwnedVariant, EditorStorageLayout.VariantsDirectoryName + "\\" + directory.DirectoryKey, "Marker-validated editor-owned variant.", variantId));
            else
        ...
        foreach (RestorePlanItem item in preview.Items)
        {
            switch (item.Kind)
            {
                ...
                case RestorePlanItemKind.RemoveOwnedVariant:
                    ...
                    VariantDirectoryOperationResult deleted = _directories.DeleteVariantDirectory(project, variant);
